...
public class RememberingWordsTraining extends JPanel implements MouseListener {
    private final ArrayList<ArrayList<String>> drawnWordsList = new ArrayList<ArrayList<String>>();
    private final ArrayList<JButton> wordButtonsList = new ArrayList<JButton>();
    ...
    public void createButtonsWithDrawnWords() throws Exception {
        ...
        this.wordButtonsList.clear();
        this.getDrawnWords();

        for (int iterator = 0; iterator < 10; iterator++) {
            JButton wordButton = new JButton(this.drawnWordsList.get(iterator).get(0));
            wordButton.addMouseListener(this);

            this.wordsGameMainPanel.add(wordButton);
            this.wordButtonsList.add(wordButton);
    ...
    private void getDrawnWords() throws Exception {
        ...
        for (int iterator = 0; iterator < 10; iterator++) {
            ...
            } while (this.drawnWordsList.contains(drawnWord));
            this.drawnWordsList.add(drawnWord);
        }
    }

    private void setCorrectAnswerLabel(int index) {
        this.correctAnswerLabel.setText(this.drawnWordsList.get(index).get(1));
        this.guessWordPanel.revalidate();
        this.guessWordPanel.repaint();
    }

    private boolean evaluateAnswer(JButton button) {
        return Objects.equals(button.getText(), this.drawnWordsList.get(this.currentCorrectAnswerIndex).get(0));
    }
    ...
    private void addSummaryOfYourAnswer(JButton button) {
        JLabel currentEnglishWord = new JLabel(this.drawnWordsList.get(this.currentCorrectAnswerIndex).get(0));
        JLabel currentPolishWord = new JLabel(this.drawnWordsList.get(this.currentCorrectAnswerIndex).get(1));
    ...
    public void mouseClicked(MouseEvent e) {
        if (this.wordButtonsList.contains(e.getSource())) {
